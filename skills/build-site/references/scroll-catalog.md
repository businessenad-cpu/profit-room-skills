...
The replication spec for a premium animated site. Every effect = **how the video is rendered** × **how scroll drives it**. The render recipe IS the effect. Get the render wrong and no front-end trick saves it.
...
Cross-cutting render rules to hand any image/video generator OR a 3D artist:
...
Front-end stack for all of them: **GSAP ScrollTrigger (pin + scrub) + Lenis (inertia) + a canvas/spritesheet flipbook + SplitType (captions).** All are free, open-source libraries — pull them from a CDN or npm.

---

## The grid

Every effect is **what drives motion** (time / scroll / cursor) × **how the video is presented** (full-bleed / masked / docked / spatial).

| Style | Drive | Presentation |
|---|---|---|
| A — Ambient loop | time | full-bleed |
| B — Scroll-scrub transform | scroll | full-bleed |
| C — Cursor-spotlight reveal | cursor | masked |
| D — Horizontal pan-lock | scroll (x) | full-bleed |
| E — Exploded / layer-peel | scroll | spatial |
| F — Camera push-through | scroll | spatial |
| G — Pinned scrollytelling | scroll | full-bleed + captions |
| + Video-in-type | time | masked (text) |
| + Shrink-to-dock (FLIP) | scroll | docked |
| + Blend-mode melt | any | masked (blend) |
| + Match-cut section transition | scroll | full-bleed |
| + Velocity-reactive | scroll velocity | any |
...
**Scroll:** Frame index = `progress * frameCount`, pinned. Drive a canvas flipbook with GSAP ScrollTrigger scrub.
**Premium gate:** Object stays itself start→end. ~120 frames over ~400vh.

## C — Cursor-Spotlight Reveal
**Render:** A still on top + the bespoke video underneath, perfectly aligned. Same framing in both.
**Drive:** A soft circular mask tracks the cursor, revealing the video beneath only inside it.
**Premium gate:** Still and video pixel-aligned; soft mask edge, not a hard circle.

## D — Horizontal Pan-Lock
...
**References:** Bulgari Eclettica (`eclettica.bulgari.com`), `the-gum.com`, `fluid.glass`, awwwards.com/websites/horizontal-layout.

## E — Exploded-View / Layer-Peel
**Why the naive attempt fails:** stacked CSS PNG layers = cardboard cut-outs. Each layer is independently lit and slides on a flat Z, so the eye reads "Photoshop." It MUST be a rendered video where all parts share one lighting environment and contact shadows.
**Render — two paths:**
...
- **AI (fast path):** generate START = assembled product, END = same product with components separated/floating apart along their axes, **identical lighting and camera**. Image-to-video the transition ("components separate apart along their axes, slight parallax orbit, locked framing, studio light, neutral bg"). Chop to frames. This is the Style-B pipeline with an exploded END instead of a rotated one.
...
## F — Camera Push-Through / Fly-Through
...
## The universal site standard

### Asset set per site (non-negotiable)
- **2 videos**: a header (hero) video + a DIFFERENT middle/second video, matched in lighting/style.
- **3 generated images**: used in the feature pairs + gallery + horizontal strip. NEVER reuse a video frame as a filler image, and never repeat the same image across panels.
...
7. **Reserve** — CTA.
8. **Footer** — animated aurora + drifting dots + the brand's own credit line.

### Universal rules
- **Hero legibility**: focused dark halo behind eyebrow + wordmark + text-shadows. Works on any footage.
- **Nav bold** (weight 500).
- **Background**: animated mesh gradient (`body::before`, slow drift), content sections translucent (90% bg) so it shows; hero footage fades into content via a seam gradient — NO abrupt solid-color jump.
- **Type scale**: thesis/section headers large.
- **Footer**: taller (min-height 240px) so aurora + dots flow smoothly.
- **Scrub heroes idle on landing**: gently drift the flipbook before first scroll, so it is alive on load (fixes "feels dead until I scroll").
- **Copy tight**: no overwrought spec lines. Less text.
- **Scrub reveal variety**: vary how content reveals during scrub (zoom-into-image-then-reveal is a strong one). Do not default to "text fades in" every time.
