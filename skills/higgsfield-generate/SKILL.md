...
- **Higgsfield access.** This skill drives the `higgsfield` CLI (installed + authed in Step 0). If the Higgsfield MCP tools are connected instead, you can route the same generations through those tools — map each `higgsfield generate create <model>` call to the equivalent MCP tool (`generate_image`, `generate_video`, `generate_audio`, `models_explore`, etc.).
- **Auth.** A logged-in Higgsfield session (`higgsfield auth login`) or a connected Higgsfield MCP. Never hard-code an API key.
- **Fallback.** If neither the CLI nor the MCP is available, tell the user to install the CLI (Step 0) or connect the Higgsfield MCP, then stop — don't fabricate result URLs.
